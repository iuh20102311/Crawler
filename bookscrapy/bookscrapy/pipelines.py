...
class CSVDBBookPipeline:
        ...
        def __init__(self):
                self.file = open('bookdata.csv', 'w', newline='', encoding='utf-8')
                self.writer = csv.writer(self.file)
                self.writer.writerow(['product_main', 'price', 'in_stock', 'availability', 'product_type', 'price_exc', 'price_inc','star', 'nor', 'tax', 'type_of_book', 'BookUrl', 'description'])

        def process_item(self, item, spider):
                values = [
                    item.get('product_main', ''),
                    item.get('price', ''),
                    item.get('in_stock', ''),
